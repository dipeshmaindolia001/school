document.addEventListener("DOMContentLoaded", () => {
  ...
  // 2. Interactive Video Reel Players with Default Audio ON
  ...
  reelCards.forEach(card => {
    ...
    if (video) {
      ...
      video.muted = false; // Default Audio ON
      ...
      const doTogglePlay = (e) => {
        if (e && e.target && e.target.closest("button.like-btn, button.reel-sound-icon")) {
          return;
        }
        if (e) {
          e.preventDefault();
        ...
        if (video.paused) {
          ...
          video.play().then(() => {
            ...
            console.log("Audio play policy fallback to muted:", err);
            video.muted = true;
            updateSoundBtnIcon();
            video.play().then(() => {
              if (centerPlay) centerPlay.style.display = "none";
